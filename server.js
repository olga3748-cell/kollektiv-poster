...
let nextClient=1,nextSticker=1,nextText=2,nextLayer=2,nextProposal=1;
const clients=new Map(),roomState=new Map(),proposals=new Map();
...
const presence=room=>roomClients(room).map(([,c])=>({id:c.id,role:c.role}));
const taken=(r,e,room)=>roomClients(room).some(([w,c])=>w!==e&&c.role===r);
function assign(w,room){return ROLES.find(x=>!taken(x.id,w,room))?.id||null}
function prop(room){const p=proposals.get(room);if(!p)return null;const members=roomClients(room).map(([,x])=>({id:x.id,role:x.role}));return {id:p.id,requester:p.requester,requestedRole:p.requestedRole,approvals:[...p.approvals],total:members.length,members}}
...
wss.on('connection',ws=>{const c={id:nextClient++,role:null,room:null};clients.set(ws,c);send(ws,{type:'hello',clientId:c.id,roles:ROLES});
 ws.on('message',raw=>{let m;try{m=JSON.parse(raw)}catch{return};
  if(m.type==='join'){c.room=String(m.room||'MAIN').trim().slice(0,40)||'MAIN';const state=getState(c.room);c.role=assign(ws,c.room);send(ws,{type:'joined',role:c.role,state,proposal:prop(c.room),clients:presence(c.room)});broadcast(c.room,{type:'presence',clients:presence(c.room)});if(proposals.has(c.room))broadcast(c.room,{type:'roleProposal',proposal:prop(c.room)});return}
  if(!c.room)return;const room=c.room,state=getState(room);
  if(m.type==='ping'){send(ws,{type:'pong'});return}
  if(m.type==='roleRequest'){const r=Number(m.requestedRole);if(!ROLES.some(x=>x.id===r)||r===c.role||proposals.has(room))return;const p={id:nextProposal++,requester:c.id,requestedRole:r,approvals:new Set([c.id])};proposals.set(room,p);broadcast(room,{type:'roleProposal',proposal:prop(room)});setTimeout(()=>{const cur=proposals.get(room);if(cur&&cur.id===p.id)resetProposal(room)},120000);return}
  ...
  const allowed={1:['bg'],2:['texture'],3:['stickers'],4:['textLayers'],5:['textLayers'],6:['draw'],7:['brush'],8:['chaos'],9:['photos']};
  if(m.type==='set'){const k=String(m.key||'');if(!(allowed[c.role]||[]).includes(k))return;if(k==='photos'&&Array.isArray(m.value)&&m.value.length>20)return;if(k==='draw'&&(!m.value||!Array.isArray(m.value.layers)))return;state[k]=Array.isArray(m.value)?m.value.slice(0,80):{...state[k],...m.value};broadcast(room,{type:'state',key:k,value:state[k]});return}
  if(m.type==='addSticker'&&c.role===3){state.stickers.push({id:nextSticker++,shape:['star','heart','circle','square','spiral'].includes(m.shape)?m.shape:'star',x:50,y:50,size:90,color:'#fff',rotation:0});broadcast(room,{type:'state',key:'stickers',value:state.stickers});return}
  ...
  if(m.type==='removePhoto'&&c.role===9){state.photos=state.photos.filter(x=>x.id!==Number(m.id));broadcast(room,{type:'state',key:'photos',value:state.photos});return}
  if(m.type==='addDrawLayer'&&c.role===6){if(state.draw.layers.length>=30)return;const l={id:nextLayer++,name:'LAGER '+(state.draw.layers.length+1),color:'#fff',strokes:[]};state.draw.layers.push(l);state.draw.activeLayer=l.id;broadcast(room,{type:'state',key:'draw',value:state.draw});return}
  if(m.type==='removeDrawLayer'&&c.role===6){if(state.draw.layers.length<=1)return;const id=Number(m.id);state.draw.layers=state.draw.layers.filter(x=>x.id!==id);if(!state.draw.layers.some(x=>x.id===state.draw.activeLayer))state.draw.activeLayer=state.draw.layers[0].id;broadcast(room,{type:'state',key:'draw',value:state.draw});return}
  if(m.type==='stroke'&&c.role===6&&Array.isArray(m.points)){const layer=state.draw.layers.find(x=>x.id===Number(m.layerId))||state.draw.layers.find(x=>x.id===state.draw.activeLayer);if(!layer)return;const pts=m.points.slice(0,900).map(p=>({x:+p.x,y:+p.y})).filter(p=>Number.isFinite(p.x)&&Number.isFinite(p.y));if(pts.length>1){const s={points:pts};layer.strokes.push(s);if(layer.strokes.length>1500)layer.strokes.shift();broadcast(room,{type:'stroke',layerId:layer.id,stroke:s})}return}
  if(m.type==='clear'&&c.role===6){state.draw.layers.forEach(l=>l.strokes=[]);broadcast(room,{type:'clear'});return}
 });
...
setInterval(()=>clients.forEach((_,w)=>w.readyState===WebSocket.OPEN&&w.ping()),25000);
server.listen(PORT,'0.0.0.0',()=>console.log('Kollektiv Poster v4.6 running on '+PORT));
