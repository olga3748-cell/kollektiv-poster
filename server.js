...
let nextClient=1,nextSticker=1,nextText=2,nextStrokeLayer=2,nextProposal=1,nextArchive=1;
const clients=new Map(),roomState=new Map(),roomMeta=new Map(),proposals=new Map();
function initialState(){return {bg:{mode:'gradient',colorA:'#101010',colorB:'#5a416f',angle:135},texture:{type:'noise',opacity:.1,color:'#fff',scale:18},stickers:[],textLayers:[{id:1,value:'COLLECTIVE SIGNAL',x:50,y:15,color:'#fff',size:42,shadow:true,font:'Arial',weight:'700',height:100,width:100,skew:0}],type:{font:'Arial',weight:'700',height:100,width:100,skew:0},draw:{color:'#fff',activeLayer:1,layers:[{id:1,name:'LAGER 1',color:'#fff',strokes:[]}],strokes:[]},brush:{size:8,opacity:.8,type:'soft',jitter:0,spacing:1},chaos:{amount:0,mode:'glitch',frequency:5},photos:[]}}
...
wss.on('connection',ws=>{const c={id:nextClient++,role:null,room:null};clients.set(ws,c);send(ws,{type:'hello',clientId:c.id,roles:ROLES});
 ...
 ws.on('message',raw=>{let m;try{m=JSON.parse(raw)}catch{return};
  ...
  if(m.type==='addDrawLayer'&&c.role===6){
    if(!state.draw.layers)state.draw.layers=[];
    const id=nextStrokeLayer++;state.draw.layers.push({id,name:'LAGER '+id,color:'#fff',strokes:[]});state.draw.activeLayer=id;
    broadcast(room,{type:'state',key:'draw',value:state.draw});return;
  }
  if(m.type==='removeDrawLayer'&&c.role===6){
    if(!state.draw.layers)state.draw.layers=[];if(state.draw.layers.length<=1)return;
  ...
  if(m.type==='setDrawLayer'&&c.role===6){
    if(!state.draw.layers)state.draw.layers=[];const l=state.draw.layers.find(x=>x.id===Number(m.id));if(!l)return;
    state.draw.activeLayer=l.id;broadcast(room,{type:'state',key:'draw',value:state.draw});return;
  }
  if(m.type==='stroke'&&c.role===6&&Array.isArray(m.points)){if(!state.draw.layers){state.draw.layers=[{id:1,name:'LAGER 1',color:state.draw.color||'#fff',strokes:state.draw.strokes||[]}];state.draw.activeLayer=1}const layer=state.draw.layers.find(x=>x.id===state.draw.activeLayer)||state.draw.layers[0];let pts=m.points.slice(0,900).map(p=>({x:+p.x,y:+p.y})).filter(p=>Number.isFinite(p.x)&&Number.isFinite(p.y));if(pts.length>1){let st={points:pts};layer.strokes.push(st);broadcast(room,{type:'stroke',layerId:layer.id,stroke:st})}return}
  if(m.type==='clear'&&c.role===6){if(state.draw.layers)state.draw.layers.forEach(l=>l.strokes=[]);state.draw.strokes=[];broadcast(room,{type:'clear'});return}
 });
